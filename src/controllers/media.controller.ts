...
export class MediaController {
	...
	@Post('create')
	@ApiOperation({ summary: 'Create media and get presigned URL for upload' })
	@ApiResponse({
		status: 201,
	...
	async markAsLoaded(@Param('mediaId') mediaId: string): Promise<{ success: boolean }> {
	...
	async downloadFile(
		...
		const { stream, contentType, contentLength } = await this.mediaService.downloadFile(url);
	...
	@ApiOperation({
		...
		description: 'Accepts file upload, processes it (streaming, no full memory load), and uploads to S3. This allows for future processing like compression, resizing, etc.'
	})
	...
	async uploadFileRedirect(
		...
		try {
			...
			await this.mediaService.uploadFileToS3(media.url, file.buffer, file.mimetype);
