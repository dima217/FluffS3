...
export class MediaService {
	...
	async createMedia(
		...
		return {
			mediaId: saved._id.toString(),url
		};
	}
	...
	async uploadFileToS3(key: string, body: Buffer, contentType?: string): Promise<void> {
	...
	async getMediaByUrl(url: string): Promise<MediaDocument> {
		...
		this.logger.debug(`Cache miss for media URL: ${url}, fetching from database`);
	...
	async downloadFile(url: string): Promise<{ stream: NodeJS.ReadableStream; contentType?: string; contentLength?: number }> {
	...
	async getUploadRedirectUrl(mediaId: string, useInternalHostname = false): Promise<string> {
		const media = await this.mediaModel.findById(mediaId).exec();
		if (!media) {
			throw new NotFoundException(`Media with ID ${mediaId} not found`);
		}
		...
		const presignedUrl = await this.s3Service.getPresignedUrl(media.url, 3600, useInternalHostname);
